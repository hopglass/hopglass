define(["moment", "numeral", "tablesort", "tablesort.numeric"],
  ...
  function showWifiChannel2(d) {
    if (!("wireless" in d.nodeinfo))
      return undefined
    if (!("chan2" in d.nodeinfo.wireless))
      return undefined

    var ch = d.nodeinfo.wireless.chan2
    return ch + " (" + wifiChannelAlias(ch) + ")"
  }

  function showWifiChannel5(d) {
    if (!("wireless" in d.nodeinfo))
      return undefined
    if (!("chan5" in d.nodeinfo.wireless))
      return undefined

    var ch = d.nodeinfo.wireless.chan5
    return ch + " (" + wifiChannelAlias(ch) + ")"
  }
  ...
  return function(config, el, router, d) {
    ...
    attributeEntry(attributes, "Wlan Kanal 2.4 GHz", showWifiChannel2(d))
    attributeEntry(attributes, "Wlan Kanal 5 GHz", showWifiChannel5(d))
    attributeEntry(attributes, "Systemlast", showLoad(d))
    attributeEntry(attributes, "Arbeitsspeicher", showRAM(d))
    attributeEntry(attributes, "Airtime 2.4 GHz", showAirtime2(d))
    attributeEntry(attributes, "Airtime 5 GHz", showAirtime5(d))
    ...
    attributeEntry(attributes, "Gewählter Nexthop", dictGet(d.statistics, ["nexthop"]))
