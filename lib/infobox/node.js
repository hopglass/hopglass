define(["moment", "numeral", "tablesort", "tablesort.numeric"],
  ...
  return function(config, el, router, d) {
    ...
    attributeEntry(attributes, "Wlan Kanal 2.4 GHz", showWifiChannel(dictGet(d.nodeinfo, ["wireless", "chan2"])))
    attributeEntry(attributes, "Wlan Kanal 5 GHz", showWifiChannel(dictGet(d.nodeinfo, ["wireless", "chan5"])))
    attributeEntry(attributes, "Systemlast", showLoad(d))
    attributeEntry(attributes, "Arbeitsspeicher", showRAM(d))
    attributeEntry(attributes, "Airtime 2.4 GHz", showAirtime(2, dictGet(d.nodeinfo, ["wireless", "airtime2"])))
    attributeEntry(attributes, "Airtime 5 GHz", showAirtime(5, dictGet(d.nodeinfo, ["wireless", "airtime5"])))
    ...
    attributeEntry(attributes, "Gewählter Nexthop", dictGet(d.nodeinfo, ["nexthop"]))
