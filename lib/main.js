...
function (moment, Router, L, GUI, numeral) {
  return function (config) {
    function handleData(data) {
      ...
      var badNodes = []
      for (i = 0; i < dataNodes.nodes.length; ++i) {
        if ("nodeinfo" in dataNodes.nodes[i])
          if (!("hostname" in dataNodes.nodes[i].nodeinfo)) {
            badNodes.push(i)
            console.log("Warning nodeinfo does not contain a hostname:")
            console.log(dataNodes.nodes[i])
          }
        if (!("nodeinfo" in dataNodes.nodes[i])) {
          badNodes.push(i)
          console.log("Warning nodeinfo not available:")
          console.log(dataNodes.nodes[i])
        }
      }
      for (i in badNodes)
        dataNodes.nodes.splice(badNodes[i], 1)
